...
Astra UI is a modular Luau ClickGUI library with an executor-ready single-file
...
Its default Astra theme uses a deep-space palette, luminous violet accents,
code-rendered tab marks, and a compact high-contrast layout.
...
- Seventeen built-in themes plus custom theme registration
...
dist/AstraUI.lua     Primary generated executor distribution
dist/ModernUI.lua    Backward-compatible copy for existing loaders
...
`dist/AstraUI.lua` without a UTF-8 byte-order mark.
...
Astra UI is available under the [MIT License](LICENSE).
